...
public class FeatureVector {
	...
	public static double dotProduct(FeatureVector fv1, FeatureVector fv2) {
		double result = 0.0;
		TIntDoubleHashMap hashMap1 = new TIntDoubleHashMap();
		TIntDoubleHashMap hashMap2 = new TIntDoubleHashMap();

		for (FeatureVector curr = fv1; curr.next != null; curr = curr.next) {
			if (curr.index < 0)
				continue;
			hashMap1.put(curr.index, hashMap1.get(curr.index) + curr.value);
		}
		for (FeatureVector curr = fv2; curr.next != null; curr = curr.next) {
			if (curr.index < 0)
				continue;
			hashMap2.put(curr.index, hashMap2.get(curr.index) + curr.value);
		}

		int[] keys = hashMap1.keys();

		for (int i = 0; i < keys.length; i++) {
			double v1 = hashMap1.get(keys[i]);
			double v2 = hashMap2.get(keys[i]);
			result += v1 * v2;
		}
	...
	// number of nodes with non-zero values
	public int sum() {
		if (next == null)
			return index >= 0 ? 1 : 0;
