...
public class DependencyDecoder2O extends DependencyDecoder {
	...
	private void rearrange(double[][][] probs, double[][][] probs_trips,
			double[][][] probs_sibs, double[][][][] nt_probs, int[] par,
			int[] labs) {
		...
		while (true) {
			...
			for (int ch = 1; ch < par.length; ch++) {
				...
				double change = 0.0
						- probs[lDir ? ch : par[ch]][lDir ? par[ch] : ch][lDir ? 1
								: 0]
						- probs_trips[par[ch]][aSib][ch]
						- probs_sibs[aSib][ch][aSib == par[ch] ? 0 : 1]
						- (bSib != ch ? probs_trips[par[ch]][ch][bSib]
								+ probs_sibs[ch][bSib][1] : 0.0)
						- (pipe.labeled ? (nt_probs[ch][labs[ch]][lDir ? 1 : 0][0] + nt_probs[par[ch]][labs[ch]][lDir ? 1
								: 0][1])
								: 0.0)
						+ (bSib != ch ? probs_trips[par[ch]][aSib][bSib]
								+ probs_sibs[aSib][bSib][aSib == par[ch] ? 0
										: 1] : 0.0);
				for (int pa = 0; pa < par.length; pa++) {
					...
					boolean lDir1 = ch < pa;
					double change1 = 0.0
							+ probs[lDir1 ? ch : pa][lDir1 ? pa : ch][lDir1 ? 1
									: 0]
							+ probs_trips[pa][aSib][ch]
							+ probs_sibs[aSib][ch][aSib == pa ? 0 : 1]
							+ (bSib != ch ? probs_trips[pa][ch][bSib]
									+ probs_sibs[ch][bSib][1] : 0.0)
							+ (pipe.labeled ? (nt_probs[ch][static_types[pa][ch]][lDir1 ? 1
									: 0][0] + nt_probs[pa][static_types[pa][ch]][lDir1 ? 1
									: 0][1])
									: 0.0)
							- (bSib != ch ? probs_trips[pa][aSib][bSib]
	...
	public Object[][] decodeNonProjective(DependencyInstance inst,
			...
			FeatureVector[][][] fvs_trips, double[][][] probs_trips,
			FeatureVector[][][] fvs_sibs, double[][][] probs_sibs,
			FeatureVector[][][][] nt_fvs, double[][][][] nt_probs, int K) {

		String[] toks = inst.sentence;
		String[] pos = inst.pos;

		Object[][] orig = decodeProjective(inst, fvs, probs, fvs_trips,
				probs_trips, fvs_sibs, probs_sibs, nt_fvs, nt_probs, 1);
		...
		for (int i = 1; i < par.length; i++) {
			...
			labs[i] = pipe.labeled ? Integer.parseInt(o[i - 1].split(":")[1])
					: 0;
		}
		...
		orig[0][0] = ((DependencyPipe2O) pipe).createFeatureVector(toks, pos,
				labs, par);
		orig[0][1] = pars;
	...
	public Object[][] decodeProjective(DependencyInstance inst,
		...
		KBestParseForest2O pf = new KBestParseForest2O(0, toks.length - 1,
				inst, K);
			...
			for (int s = 0; s < toks.length && s + j < toks.length; s++) {
				...
				if (true) {
					...
					if (!(b1 == null || c1 == null)) {
						...
						FeatureVector prodFV_sst = pf.cat(fvs_trips[s][s][t],
								fvs_sibs[s][t][0]);
						double prodProb_sst = probs_trips[s][s][t]
								+ probs_sibs[s][t][0];
						...
						for (int k = 0; k < pairs.length; k++) {
							...
							FeatureVector fv_fin = pf
									.cat(prodFV_st, prodFV_sst);
							if (pipe.labeled) {
								bc += nt_prob_s_01 + nt_prob_t_00;
								fv_fin = FeatureVector.cat(nt_fv_s_01,
										FeatureVector.cat(nt_fv_t_00, fv_fin));
							}

							pf.add(s, s, t, type1, 0, 1, bc, fv_fin, b1[comp1],
									c1[comp2]);
					...
					if (!(b1 == null || c1 == null)) {
						...
						FeatureVector prodFV_stt = pf.cat(fvs_trips[t][t][s],
								fvs_sibs[t][s][0]);
						double prodProb_stt = probs_trips[t][t][s]
								+ probs_sibs[t][s][0];
						...
						for (int k = 0; k < pairs.length; k++) {
							...
							FeatureVector fv_fin = pf
									.cat(prodFV_ts, prodFV_stt);
							if (pipe.labeled) {
								bc += nt_prob_t_11 + nt_prob_s_10;
								fv_fin = FeatureVector.cat(nt_fv_t_11,
										FeatureVector.cat(nt_fv_s_10, fv_fin));
							}

							pf.add(s, t, t, type2, 1, 1, bc, fv_fin, b1[comp1],
									c1[comp2]);
						}
					}
				}
				...
				for (int r = s; r < t; r++) {
					...
					if (!(b1 == null || c1 == null)) {
						...
						for (int k = 0; k < pairs.length; k++) {
							...
							pf.add(s, r, t, -1, 0, 2, bc, new FeatureVector(-1,
									-1.0, null), b1[comp1], c1[comp2]);
							pf.add(s, r, t, -1, 1, 2, bc, new FeatureVector(-1,
									-1.0, null), b1[comp1], c1[comp2]);
						}
					}
				}
				...
				for (int r = s + 1; r < t; r++) {
					...
					if (!(b1 == null || c1 == null)) {
						...
						for (int k = 0; k < pairs.length; k++) {
							...
							bc += prodProb_st + probs_trips[s][r][t]
									+ probs_sibs[r][t][1];
							FeatureVector fv_fin = pf.cat(prodFV_st, pf.cat(
									fvs_trips[s][r][t], fvs_sibs[r][t][1]));
							...
							pf.add(s, r, t, type1, 0, 1, bc, fv_fin, b1[comp1],
									c1[comp2]);
						}
					}
					...
					if (!(b1 == null || c1 == null)) {
						...
						for (int k = 0; k < pairs.length; k++) {
							...
							bc += prodProb_ts + probs_trips[t][r][s]
									+ probs_sibs[r][s][1];

							FeatureVector fv_fin = pf.cat(prodFV_ts, pf.cat(
									fvs_trips[t][r][s], fvs_sibs[r][s][1]));
							if (pipe.labeled) {
								bc += nt_prob_t_11 + nt_prob_s_10;
								fv_fin = FeatureVector.cat(nt_fv_t_11,
										FeatureVector.cat(nt_fv_s_10, fv_fin));
							...
							pf.add(s, r, t, type2, 1, 1, bc, fv_fin, b1[comp1],
									c1[comp2]);
						}
					}
