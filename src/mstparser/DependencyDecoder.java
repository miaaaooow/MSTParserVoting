...
public class DependencyDecoder {
					...
					if (r != t) {
						...
						if (b1 != null && c1 != null) {
							...
							for (int k = 0; k < pairs.length; k++) {
								...
								if (pipe.labeled) {
									fv_fin = nt_fv_s_01.cat(nt_fv_t_00
											.cat(fv_fin));
									prob_fin += nt_prob_s_01 + nt_prob_t_00;
								}
								pf.add(s, r, t, type1, 0, 1, prob_fin, fv_fin,
										b1[comp1], c1[comp2]);

								prob_fin = bc + prodProb_ts;
								fv_fin = prodFV_ts;
								if (pipe.labeled) {
									fv_fin = nt_fv_t_11.cat(nt_fv_s_10
											.cat(fv_fin));
									prob_fin += nt_prob_t_11 + nt_prob_s_10;
								}
								pf.add(s, r, t, type2, 1, 1, prob_fin, fv_fin,
										b1[comp1], c1[comp2]);
	...
	public Object[][] decodeNonProjective(DependencyInstance inst,
		...
		for (int i = 0; i < numWords; i++) {
			...
			for (int j = 0; j < numWords; j++) {
				...
				scoreMatrix[i][j] = probs[i < j ? i : j][i < j ? j : i][i < j ? 0
						: 1]
						+ (pipe.labeled ? nt_probs[i][static_types[i][j]][i < j ? 0
								: 1][1]
								+ nt_probs[j][static_types[i][j]][i < j ? 0 : 1][0]
								: 0.0);
				orig_scoreMatrix[i][j] = probs[i < j ? i : j][i < j ? j : i][i < j ? 0
						: 1]
						+ (pipe.labeled ? nt_probs[i][static_types[i][j]][i < j ? 0
								: 1][1]
								+ nt_probs[j][static_types[i][j]][i < j ? 0 : 1][0]
		...
		for (int k = 0; k < fin_par.length; k++) {
			for (int i = 0; i < fin_par[k].length; i++) {
				...
				int pr = fin_par[k][i];
				if (pr != -1) {
					fin_fv[k][ch] = fvs[ch < pr ? ch : pr][ch < pr ? pr : ch][ch < pr ? 1
							: 0];
					if (pipe.labeled) {
						fin_fv[k][ch] = fin_fv[k][ch]
								.cat(nt_fvs[ch][static_types[pr][ch]][ch < pr ? 1
										: 0][0]);
						fin_fv[k][ch] = fin_fv[k][ch]
								.cat(nt_fvs[pr][static_types[pr][ch]][ch < pr ? 1
										: 0][1]);
					}
				} else
		...
		for (int k = 0; k < fin.length; k++) {
			...
			for (int i = 1; i < par.length; i++)
				result[k] += fin_par[k][i]
						+ "|"
						+ i
						+ (pipe.labeled ? ":" + static_types[fin_par[k][i]][i]
								: ":0") + " ";
		}
	...
	public static TIntIntHashMap chuLiuEdmonds(double[][] scoreMatrix,
		...
		if (print) {
			int[] keys = finalEdges.keys();
			Arrays.sort(keys);
			for (int i = 0; i < keys.length; i++)
				System.out
						.print(finalEdges.get(keys[i]) + "|" + keys[i] + " ");
			System.out.println();
		}
