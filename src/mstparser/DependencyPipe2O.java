...
public class DependencyPipe2O extends DependencyPipe {
	...
	public FeatureVector createFeatureVector(String[] toks, String[] pos,
			String[] posA, int par, int ch1, int ch2, FeatureVector fv) {
	...
	public FeatureVector createFeatureVectorSib(String[] toks, String[] pos,
			int ch1, int ch2, boolean isST, FeatureVector fv) {
		...
		fv = add("CH_PAIR_DIST=" + ch1_pos + "_" + ch2_pos + "_" + distBool
				+ "_" + dir, 1.0, fv);
		fv = add("ACH_PAIR_DIST=" + ch1_pos + "_" + ch2_pos + "_" + distBool,
				1.0, fv);
	...
	public FeatureVector createFeatureVector(String[] toks, String[] pos,
		...
		for (int i = 0; i < toks.length; i++) {
			...
			if (labeled) {
				fv = createFeatureVector(toks, pos, posA, i, labs[i], attR,
						true, fv);
				fv = createFeatureVector(toks, pos, posA, deps[i], labs[i],
						attR, false, fv);
			}
		}
		...
		for (int i = 0; i < toks.length; i++) {
			...
			for (int j = i + 1; j < toks.length; j++) {
				if (deps[j] == i) {
					...
					fv = createFeatureVectorSib(toks, pos, prev, j, prev == i,
							fv);
					prev = j;
				}
			}
	 ...
	 * NewFeatureVector[][][] fvs, NewFeatureVector[][][] fvs_trips,
	 * NewFeatureVector[][][] fvs_sibs, NewFeatureVector[][][] fvs_gp) {
	 * 
	 * FeatureVector fv = new FeatureVector(-1,-1.0,null); for(int i = 0; i <
	 * deps.length; i++) { if(deps[i] == -1) continue; int small = i < deps[i] ?
	 * i : deps[i]; int large = i > deps[i] ? i : deps[i]; int attR = i <
	 * deps[i] ? 1 : 0; fv = NewFeatureVector.cat(fvs[small][large][attR],fv); }
	 * // find all trip features for(int i = 0; i < deps.length; i++) { // right
	 * children if(deps[i] == -1 && i != 0) continue; int prev = i; for(int j =
	 * i+1; j < deps.length; j++) { if(deps[j] == i) { fv =
	 * NewFeatureVector.cat(fvs_trips[i][prev][j],fv); fv =
	 * NewFeatureVector.cat(fvs_sibs[prev][j][prev == i ? 0 : 1],fv); prev = j;
	 * } } prev = i; for(int j = i-1; j >= 0; j--) { if(deps[j] == i) { fv =
	 * NewFeatureVector.cat(fvs_trips[i][prev][j],fv); fv =
	 * NewFeatureVector.cat(fvs_sibs[prev][j][prev == i ? 0 : 1],fv); prev = j;
	 * } } }
	 * 
				...
				for (int w2 = w1 + 1; w2 < toks.length; w2++) {
					...
					for (int ph = 0; ph < 2; ph++) {
						...
						FeatureVector prodFV = createFeatureVector(toks, pos,
								posA, w1, w2, attR, new FeatureVector(-1, -1.0,
										null));
			...
			for (int w1 = 0; w1 < toks.length; w1++) {
				for (int w2 = w1; w2 < toks.length; w2++) {
					for (int w3 = w2 + 1; w3 < toks.length; w3++) {
						FeatureVector prodFV = createFeatureVector(toks, pos,
								posA, w1, w2, w3, new FeatureVector(-1, -1.0,
										null));
						for (FeatureVector curr = prodFV; curr != null; curr = curr.next) {
							if (curr.index >= 0)
								out.writeInt(curr.index);
			...
			for (int w1 = 0; w1 < toks.length; w1++) {
				for (int w2 = 0; w2 < toks.length; w2++) {
					for (int wh = 0; wh < 2; wh++) {
						if (w1 != w2) {
							FeatureVector prodFV = createFeatureVectorSib(toks,
									pos, w1, w2, wh == 0, new FeatureVector(-1,
											-1.0, null));
							for (FeatureVector curr = prodFV; curr != null; curr = curr.next) {
								if (curr.index >= 0)
									out.writeInt(curr.index);
	...
	public DependencyInstance getFeatureVector(ObjectInputStream in,
			...
			double[][][] probs, FeatureVector[][][] fvs_trips,
			double[][][] probs_trips, FeatureVector[][][] fvs_sibs,
			double[][][] probs_sibs, FeatureVector[][][][] nt_fvs,
	...
	public void getFeatureVector(DependencyInstance inst,
		...
		for (int w1 = 0; w1 < toks.length; w1++) {
			for (int w2 = w1; w2 < toks.length; w2++) {
				for (int w3 = w2 + 1; w3 < toks.length; w3++) {
					FeatureVector prodFV = createFeatureVector(toks, pos, posA,
							w1, w2, w3, new FeatureVector(-1, -1.0, null));
					double prodProb = params.getScore(prodFV);
					fvs_trips[w1][w2][w3] = prodFV;
					probs_trips[w1][w2][w3] = prodProb;
				}
