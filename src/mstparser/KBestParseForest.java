...
//		this.pos = inst.pos;
		...
		for (int i = 0; i < K; i++) {
			if (chart[s][s][dir][0][i].prob < score) {
				...
				chart[s][s][dir][0][i] = new ParseForestItem(s, type, dir,
						score, fv);
				for (int j = i + 1; j < K
						&& tmp.prob != Double.NEGATIVE_INFINITY; j++) {
	...
	public boolean add(int s, int r, int t, int type, int dir, int comp,
			double score, FeatureVector fv, ParseForestItem p1,
			ParseForestItem p2) {
		...
		if (chart[s][t][dir][comp][0] == null) {
			for (int i = 0; i < K; i++)
				chart[s][t][dir][comp][i] = new ParseForestItem(s, r, t, type,
						dir, comp, Double.NEGATIVE_INFINITY, null, null, null);
		}
	...
	public String getDepString(ParseForestItem pfi) {
		...
		if (pfi.comp == 0) {
			return (getDepString(pfi.left) + " " + getDepString(pfi.right))
					.trim();
		} else if (pfi.dir == 0) {
			return ((getDepString(pfi.left) + " " + getDepString(pfi.right))
					.trim() + " " + pfi.s + "|" + pfi.t + ":" + pfi.type)
					.trim();
		} else {
	...
	public int[][] getKBestPairs(ParseForestItem[] items1,
		...
		int n = 0;
		ValueIndexPair vip = new ValueIndexPair(
				items1[0].prob + items2[0].prob, 0, 0);
