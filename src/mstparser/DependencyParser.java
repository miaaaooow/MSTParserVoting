...
public class DependencyParser {
	...
	public static int votingModels = -1;
	...
	public DependencyParser(DependencyPipe pipe) {
		...
		decoder = secondOrder ? new DependencyDecoder2O(pipe)
				: new DependencyDecoder(pipe);
	}

	/**
	 * Invoke all training iterations
	 * @param depInstances
	 * @param trainFile
	 * @param trainForest
	 * @throws IOException
	 */
	public void train(DependencyInstance[] depInstances, String trainFile,
			String trainForest) throws IOException {
		...
		for (i = 0; i < numIters; i++) {
			...
			trainingIter(depInstances, trainFile, trainForest, i + 1);
		...
		params.averageParams(i * depInstances.length);
	 ...
	 * @throws IOException
	 */
	private void trainingIter(DependencyInstance[] trees, String trainFile,
			String trainForest, int iter) throws IOException {
		...
		for (int i = 0; i < trees.length; i++) {
			...
			DependencyInstance inst = trees[i];
			...
			if (secondOrder)
				inst = ((DependencyPipe2O) pipe).getFeatureVector(in, inst,
						fvs, probs, fvs_trips, probs_trips, fvs_sibs,
						probs_sibs, nt_fvs, nt_probs, params);
			else
				inst = pipe.getFeatureVector(in, inst, fvs, probs, nt_fvs,
						nt_probs, params);

			double upd = (double) (numIters * trees.length	- (trees.length * (iter - 1) + (i + 1)) + 1);
			int K = trainK;
			Object[][] d = null;
			/** PROJECTIVE **/
			if (decodeType.equals("proj")) {
				if (secondOrder) {
					d = ((DependencyDecoder2O) decoder).decodeProjective(inst,
							fvs, probs, fvs_trips, probs_trips, fvs_sibs,
			...
			if (decodeType.equals("non-proj")) {
				if (secondOrder)
					d = ((DependencyDecoder2O) decoder).decodeNonProjective(
							inst, fvs, probs, fvs_trips, probs_trips, fvs_sibs,
							probs_sibs, nt_fvs, nt_probs, K);
				else
					d = decoder.decodeNonProjective(inst, fvs, probs, nt_fvs,
							nt_probs, K);
			}
		...
		System.out.println("  " + trees.length + " instances");
	...
	// Get Best Parses ///////////////////////////////////
	// ////////////////////////////////////////////////////
	public void outputParses(String tFile, String file) throws IOException {

		long start = System.currentTimeMillis();

		BufferedWriter pred = new BufferedWriter(new OutputStreamWriter(
				new FileOutputStream(file), "UTF8"));

		BufferedReader in = new BufferedReader(new InputStreamReader(
				new FileInputStream(tFile), "UTF8"));
		...
		while (il != null) {
			...
			if (decodeType.equals("proj")) {
				if (secondOrder) {
					d = ((DependencyDecoder2O) decoder).decodeProjective(il,
							fvs, probs, fvs_trips, probs_trips, fvs_sibs,
							probs_sibs, nt_fvs, nt_probs, K);
				}
				else {
					d = decoder.decodeProjective(il, fvs, probs, nt_fvs,
							nt_probs, K);
				}
			}
			if (decodeType.equals("non-proj")) {
				if (secondOrder)
					d = ((DependencyDecoder2O) decoder).decodeNonProjective(il,
							fvs, probs, fvs_trips, probs_trips, fvs_sibs,
							probs_sibs, nt_fvs, nt_probs, K);
				else
					d = decoder.decodeNonProjective(il, fvs, probs, nt_fvs,
							nt_probs, K);
			}
	...
	public static void processArguments(String[] args) {
		for (int i = 0; i < args.length; i++) {
			...
			if (pair[0].equals("voing-mode")) {
				votingMode = pair[1].equals("true") ? true : false;
			}
			if (pair[0].equals("voing-models")) {
				votingModels = Integer.parseInt(pair[1]);
			}
		}
		...
		System.out.println("voting-mode: " + votingMode);		
		System.out.println("voting-models: " + votingModels);
		System.out.println("------\n");
	}
	
	// SET TEST ARGUMENTS
	public static String [] setArguments() {
		String mstDir = "/home/mateva/FMI/MastersThesis/MSTParser/MSTParser/";
		String [] argz = { 	"train",
				"train-file:" + mstDir + "btb/BTB-02-Train.mst",
				"model-name:" + mstDir + "outputs/voting-all-1.dep",
				"output-file:" + mstDir + "outputs/voting-all-1.txt",
	...
	public static void main(String[] args) throws FileNotFoundException,
		...
		processArguments(setArguments());

		if (train) {

			DependencyPipe pipe = secondOrder ? new DependencyPipe2O(
					createForest) : new DependencyPipe(createForest);

			pipe.setLabeledAndVoting(trainfile, votingMode);
		...
		if (test) {
			DependencyPipe pipe = secondOrder ? new DependencyPipe2O(true)
					: new DependencyPipe(true);
			pipe.setLabeledAndVoting(testfile, votingMode);
			DependencyParser dp = new DependencyParser(pipe);
